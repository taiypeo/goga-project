...
sched = BackgroundScheduler(job_stores={
    'default': SQLAlchemyJobStore(engine=alchemy_engine)
})
...
class Event(Base):
    ...
    def __repr__(self):
        ...
        if user_count != 1:
            return f"<Event #{self.id} users: {user_count} date={self.date}{expired_str}>"
        else:
            return f"<Event #{self.id} to: {self.users[0]} date={self.date}{expired_str}>"
